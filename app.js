(() => {
  ...
  // Stable IDs and known spelling variants identify only the requested removals.
  const REMOVED_WORKER_IDS = ['C00-01', 'C01-05', 'C01-02'];
  function removedWorker(p) {
    const name = String(p.name || '').trim().replace(/\s+/g, ' ').toLowerCase();
    return REMOVED_WORKER_IDS.includes(String(p.id)) ||
      ['lungelo dlamini', 'rafeeq batjies', 'rafeeq baatjies', 'patrick zuma', 'patrick jdsn'].includes(name);
  }
  function removedWorkerIds(data) {
    return new Set([...REMOVED_WORKER_IDS, ...(data.people || []).filter(removedWorker).map(p => String(p.id))]);
  }
  function removeRequestedWorkers(data) {
    const ids = removedWorkerIds(data);
    data.people = (data.people || []).filter(p => !ids.has(String(p.id)));
    METRICS.forEach(m => { data[m] = (data[m] || []).filter(r => !ids.has(String(r.personId))); });
    return data;
  }
  ...
  function loadLocal() {
    try {
      ...
      return removeRequestedWorkers(parsed);
  ...
  function saveLocal() { removeRequestedWorkers(db); localStorage.setItem(STORAGE_KEY, JSON.stringify(db)); }
  ...
  function reconcileAdminRoster(cloudPeople) {
    const people = Array.isArray(cloudPeople) ? cloudPeople : [];
    // Preserve every remaining worker exactly as received, including recent edits.
    return { people: people.filter(p => !removedWorker(p)), changed: people.some(removedWorker) };
  }
  ...
  async function syncFromRemote(showMessage=true) {
    ...
    try{
      ...
      if(data.data){
        ...
        const removedIds=removedWorkerIds(incoming);
        ...
        if(session?.user?.role==='admin') {
          try {
            // Delete only these workers' records; never replace another worker's data.
            for (const metric of METRICS) {
              for (const record of (data.data[metric] || [])) {
                if(removedIds.has(String(record.personId))) await remoteCall({action:'deleteRecord',metric,id:record.id});
              }
            }
            if(rosterFix.changed) await remoteCall({action:'savePeople',people:db.people});
          } catch(e) {
            toast('The three workers are excluded locally, but their cloud removal could not finish: '+e.message,'error');
            throw e;
          }
        }
